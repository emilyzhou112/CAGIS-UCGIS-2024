...
criteriaMinMax <- c("min", "max", "max", "min", "max", "min", "min", "max", "max")
overall_c1 <- TOPSIS(performanceTable_c1, weights, criteriaMinMax)
...
final_percentile <- final %>% 
  dplyr::select(NAME, uniqueID, avg_rank, topsis_rank) %>%
  mutate(
...
# quick check
TOPSIS_c1 %>% 
  left_join(final, by = "uniqueID") %>%
  dplyr::select(uniqueID, NAME, topsis_rank_c1, topsis_percentile_c1, topsis_rank, topsis_percentile)
```
