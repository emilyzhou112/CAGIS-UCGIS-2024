...
From here, we can see that each cluster has different characteristics. For example, Cluster 1 has the highest average population density, driving population, registered EV, are more accessible to existing EVSE, and have more parking spaces nearby. 

Cluster 4 also has high population density, and more than half of the population are in driving age. It has a fair number of registered electric vehicle, electric power usage is huge. Accessibility to the existing EVSE is low. 

Cluster 3 has moderate population density, but also more than half of the population are in driving age. It has a lot of registered electric vehicle, but accessibility to EVSE is low. 

Cluster 2 has low density and driving age population, but does have electric vehicle registration.

Looking at the map and based on our knowledge about Philadelphia, cluster 1 is likely to be Center City and University City, cluster 4 is likely to be Northeast Philadelphia and west Philadelphia, cluster 2 and 3 are both surban area, but cluster 3 is likely to be more affluent area while cluster 2 covers a lot of industrial area.
...
We will use the TOPSIS method to test out the local MCDA model
...
we need to find a way to summarize the result of the local MCDA within each cluster.Probably the percentile rank of each grid cell within each cluster, compared to their average percentile rank in the entire city using global MCDA. 
