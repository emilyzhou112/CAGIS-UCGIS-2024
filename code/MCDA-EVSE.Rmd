...
ggplot()+
  ...
  geom_sf(data=points_sf %>% st_buffer(dist = 1640), color="black")
...
temp <- st_join(net_centroid, electricity %>% 
                  st_buffer(dist = 1640)) %>% 
  group_by(uniqueID.x) %>% 
...
Now, this is based solely on quantitative analysis on the distribution of the data. 
```{r}
...
```
```{r}
range(performanceTable$evse.nn) 
range(performanceTable$density) / 4
range(performanceTable$driving_pop) / 4
range(performanceTable$TSFCA) / 4 
range(performanceTable$elec_cnt) / 4
range(performanceTable$parking.nn) / 4
range(performanceTable$policefire.nn) / 4
range(performanceTable$zone_score) / 4
```


```{r}

criteriaWeights <- c(0.2, 0.15, 0.1, 0.15, 0.2, 0.1, 0.05, 0.05)
names(criteriaWeights)<-colnames(performanceTable)

criteriaMinMax <- c("min", "max", "max", "min", "max", "min", "min", "max")
names(criteriaMinMax)<-colnames(performanceTable)

preferenceFunction<-c("Usual","U-shape","V-shape","Level","V-shape-Indiff","Gaussian", "Level","V-shape-Indiff")
gaussParameter<-c(0.25,1,2,0,0,0,0,0)
names(gaussParameter)<-colnames(performanceTable)

#Preference threshold
preferenceThreshold<-c(0.31, 0.21, 0.12, 0, 0.17, 0.19, 0.23, 0.075)
names(preferenceThreshold)<-colnames(performanceTable)

#Indifference threshold
indifferenceThreshold<-c(0.1, 0.07, 0.041, 0, 0.058, 0.065, 0.078, 0.025)
...
ggplot()+
  geom_sf(data=fishnet, color="black", fill="white") +
  geom_sf(data=PROMETHEE_toplot, color="black", aes(fill=net))
```
