...
WSM_c1 <- cluster1 %>% 
  ...
  dplyr::select(c(uniqueID, score)) %>% 
  mutate(wsm_rank_c1 = min_rank(desc(score))) %>% 
...
WSM_c2 <- cluster2 %>% 
  ...
  dplyr::select(c(uniqueID, score)) %>%
  mutate(wsm_rank_c2 = min_rank(desc(score))) %>%
...
WSM_c3 <- cluster3 %>% 
  ...
  dplyr::select(c(uniqueID, score)) %>%
  mutate(wsm_rank_c3 = min_rank(desc(score))) %>%
...
WSM_c4 <- cluster4 %>% 
  ...
  dplyr::select(c(uniqueID, score)) %>%
  mutate(wsm_rank_c4 = min_rank(desc(score))) %>%
...
WSM_c1_org <- cluster1 %>% 
  ...
  dplyr::select(c(uniqueID, score)) %>% 
  mutate(wsm_rank_c1_org = min_rank(desc(score))) %>% 
...
WSM_c2_org <- cluster2 %>%
  ...
  mutate(score = 0.18*scl_evse_re + 0.12*scl_density + 0.1*scl_drivingpop + 0.12*scl_acc_re + 0.15*scl_elec + 0.1*scl_parking_re + 0.05*scl_policefire_re + 0.05*zone_score + 0.13*scl_elect_use) %>% 
  dplyr::select(c(uniqueID, score)) %>%
  mutate(wsm_rank_c2_org = min_rank(desc(score))) %>%
...
WSM_c3_org <- cluster3 %>%
  ...
  mutate(score = 0.18*scl_evse_re + 0.12*scl_density + 0.1*scl_drivingpop + 0.12*scl_acc_re + 0.15*scl_elec + 0.1*scl_parking_re + 0.05*scl_policefire_re + 0.05*zone_score + 0.13*scl_elect_use) %>% 
  dplyr::select(c(uniqueID, score)) %>%
  mutate(wsm_rank_c3_org = min_rank(desc(score))) %>%
...
WSM_c4_org <- cluster4 %>%
  ...
  mutate(score = 0.18*scl_evse_re + 0.12*scl_density + 0.1*scl_drivingpop + 0.12*scl_acc_re + 0.15*scl_elec + 0.1*scl_parking_re + 0.05*scl_policefire_re + 0.05*zone_score + 0.13*scl_elect_use) %>% 
  dplyr::select(c(uniqueID, score)) %>%
  mutate(wsm_rank_c4_org = min_rank(desc(score))) %>%
...
WSM_c3_cp %>% 
  ggplot(aes(x = wsm_percentile , y = wsm_percentile_c3)) +
  geom_point() +
...
model1_wsm <- lm(wsm_percentile_c4 ~ wsm_percentile, data = WSM_c4_cp)
model2_wsm<- lm(wsm_percentile_c4 ~ wsm_percentile_c4_org, data = WSM_c4_cp)
model3_wsm <- lm(wsm_percentile_c4_org ~ wsm_percentile , data = WSM_c4_cp)
...
Cluster 1: 
Local TOPSIS (Local Weights) vs. Global TOPSIS --- 2.800 (0.787 Rsq)
Local TOPSIS (Global Weights) vs. Local TOPSIS (Local Weights) --- 0.973 (0.947 Rsq)
Local TOPSIS (Global Weights) vs. Global TOPSIS --- 0.269 (0.724 Rsq)


Cluster 2: 
Local TOPSIS (Local Weights) vs. Global TOPSIS --- 1.240 (0.795 Rsq)
Local TOPSIS (Global Weights) vs. Local TOPSIS (Local Weights) --- 0.987 (0.975 Rsq)
Local TOPSIS (Global Weights) vs. Global TOPSIS --- 0.654 (0.821 Rsq)

Cluster 3:
Local TOPSIS (Local Weights) vs. Global TOPSIS --- 1.060 (0.904 Rsq)
Local TOPSIS (Global Weights) vs. Local TOPSIS (Local Weights) --- 0.982 (0.965 Rsq)
Local TOPSIS (Global Weights) vs. Global TOPSIS --- 0.841 (0.879 Rsq) 

Cluster 4: 
Local TOPSIS (Local Weights) vs. Global TOPSIS --- 1.220 (0.725 Rsq)
Local TOPSIS (Global Weights) vs. Local TOPSIS (Local Weights) --- 0.977 (0.954 Rsq)
Local TOPSIS (Global Weights) vs. Global TOPSIS --- 0.609 (0.76 Rsq)

High R² (0.947–0.975) for Local MCDA (Global Weights) vs. Local MCDA (Local Weights) across all clusters suggests that applying local weights slightly modifies but does not drastically change rankings. 
Lower slopes and R² for Local MCDA (Local Weights) vs. Global MCDA (especially in Cluster 1 and Cluster 4) indicate that the most significant rank shifts occur when transitioning from a global model to a local model.

In cluster 1, the slope of 2.800 for Local (Local Weights) vs. Global MCDA suggests extreme shifts in ranking, with certain grids ranking much higher or much lower locally than globally. Locally, some areas within the core have high demand but remain underserved, which global analysis fails to highlight.

In cluster 2, the slope for Local (Local Weights) vs. Global MCDA is 1.240 with R² of 0.795, suggesting that while local factors alter rankings, the overall prioritization is more aligned with the global MCDA compared to Cluster 1.

In cluster 3, the slope of 1.060 for Local (Local Weights) vs. Global MCDA, with an R² of 0.904, indicates minimal ranking disruptions between global and local approaches.

In cluster 4, the slope for Local (Local Weights) vs. Global MCDA is 1.220 with an R² of 0.725, suggesting substantial local ranking shifts. Additionally, the slope of 0.609 for Global MCDA vs. Local MCDA (Global Weights) is among the lowest, indicating that local constraints heavily influence rankings.

The choice of local vs. global scale significantly alters rankings, particularly in areas with high spatial and socioeconomic variability (e.g., Cluster 1 and Cluster 4).Localized MCDA is crucial for equity-driven EVSE planning, as global approaches may overlook accessibility barriers in disadvantaged communities. Areas with high demand but infrastructure gaps (e.g., Cluster 1 and Cluster 4) require localized adjustments.
...
global_local <- TOPSIS %>% 
  ...
  left_join(TOPSIS_c1 %>% mutate(c1_val = values) %>% dplyr::select(uniqueID, c1_val), by = "uniqueID") %>%
  left_join(TOPSIS_c2 %>% mutate(c2_val = values) %>% dplyr::select(uniqueID, c2_val), by = "uniqueID") %>%
  left_join(TOPSIS_c3 %>% mutate(c3_val = values) %>% dplyr::select(uniqueID, c3_val), by = "uniqueID") %>%
  left_join(TOPSIS_c4 %>% mutate(c4_val = values) %>% dplyr::select(uniqueID, c4_val), by = "uniqueID") %>%
  mutate(local_val = coalesce(c1_val, c2_val, c3_val, c4_val)) %>%
  mutate(local_rank = min_rank(desc(local_val))) %>%
  dplyr::select(uniqueID, global_val, global_rank, local_val, local_rank)
...
quantiles_glo <- classIntervals(global_local_toplot$global_val, n = 6, style = "quantile")
quantiles_loc <- classIntervals(global_local_toplot$local_val, n = 6, style = "quantile")
...
ggplot()+
  geom_sf(data=fishnet, color="black", fill="white") +
 geom_sf(data = global_local_toplot, aes(fill = quantile_loc), color = "black") +
  scale_fill_brewer(palette = "Blues") + 
...
TOPSIS_c1_cp %>% 
  ggplot(aes(x = topsis_percentile_c1_org, y = topsis_percentile)) +
  geom_point() +
...
model1 <- lm(topsis_percentile_c1 ~ topsis_percentile, data = TOPSIS_c1_cp)
model2<- lm(topsis_percentile_c1 ~ topsis_percentile_c1_org, data = TOPSIS_c1_cp)
model3 <- lm(topsis_percentile_c1_org ~ topsis_percentile, data = TOPSIS_c1_cp)
...
Cluster 1: 
Local TOPSIS (Local Weights) vs. Global TOPSIS --- 0.870 (0.305 Rsq)
Local TOPSIS (Global Weights) vs. Local TOPSIS (Local Weights) --- 0.901 (0.811 Rsq)
Local TOPSIS (Global Weights) vs. Global TOPSIS --- 0.207 (0.106 Rsq)


Cluster 2: 
Local TOPSIS (Local Weights) vs. Global TOPSIS --- 0.979 (0.868 Rsq)
Local TOPSIS (Global Weights) vs. Local TOPSIS (Local Weights) --- 0.930 (0.865 Rsq)
Local TOPSIS (Global Weights) vs. Global TOPSIS --- 0.802 (0.711 Rsq)

Cluster 3:
Local TOPSIS (Local Weights) vs. Global TOPSIS --- 0.853 (0.727 Rsq)
Local TOPSIS (Global Weights) vs. Local TOPSIS (Local Weights) --- 0.911 (0.830 Rsq)
Local TOPSIS (Global Weights) vs. Global TOPSIS --- 0.890 (0.794 Rsq)

Cluster 4: 
Local TOPSIS (Local Weights) vs. Global TOPSIS --- 0.837 (0.416 Rsq)
Local TOPSIS (Global Weights) vs. Local TOPSIS (Local Weights) --- 0.962 (0.925 Rsq)
Local TOPSIS (Global Weights) vs. Global TOPSIS --- 0.491 (0.406)
...
Clusters 1 and 4 exhibit significant local variation, suggesting global decision models require substantial adaptation before implementation.
A hybrid decision-making approach—where global models are used as a reference but adapted based on local weightings—may be optimal in regions with moderate alignment (e.g., Cluster 3).

See an example here: 
842 Frankford. in Cluster 4
Ranked 45 in Global
Ranked 4.35 in Local
...
```{r sankey, fig.height=14, fig.width=14}
...
TOPSIS_c4_cp %>%
  mutate(
    ...
    local_global_weight_rank = cut(topsis_percentile_c4, breaks = c(-1, 20, 40, 60, 80, 101), labels = c("0-20%", "20-40%", "40-60%", "60-80%", "80-100%")),
    local_local_weight_rank = cut(topsis_percentile_c4_org, breaks = c(-1, 20, 40, 60, 80, 101), labels = c("0-20%", "20-40%", "40-60%", "60-80%", "80-100%"))
  ) %>%
...
**We need to develop a way to compute for example how many grids in a cluster were ranked below top 20% in global MCDA but were not in the top 20% in local MCDA. We want to see the size of the effect from global to local.**
...
cluster4 %>% 
  left_join(TOPSIS, by = "uniqueID") %>%
  dplyr::select(uniqueID, topsis_rank) %>%
  summarise(mean_rank = mean(topsis_rank))
```
...
In Cluster 1, there were 9 out of 246 grids ranked not in the top 20% in the global model but were in the top 20% in the local model.
In Cluster 2, there were 18 out of 190 grids ranked not in the top 20% in the global model but were in the top 20% in the local model.
In Cluster 3, there were 30 out of 269 grids ranked not in the top 20% in the global model but were in the top 20% in the local model.
In Cluster 4, there were 85 out of 507 grids ranked not in the top 20% in the global model but were in the top 20% in the local model. 
...
TOPSIS_c4_cp %>%
  ...
  filter(global_topsis_rank == "0-20%")
 # filter(global_topsis_rank != "0-20%" & local_local_weight_rank == "0-20%")
```
...
**Decision-makers need to engage with local stakeholders when applying global models to ensure policies remain relevant. If policies are based on global TOPSIS rankings alone, they may overlook specific local priorities, leading to ineffective or misaligned interventions.**
